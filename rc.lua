...
mytheme = awful.util.getdir("config").."/themes/zenburn/theme.lua"
...
myarchmenu = {
  { "rc.conf", su_editor_cmd .. " /etc/rc.conf"},
  { "mkinitcpio.conf", su_editor_cmd .. " /etc/mkinitcpio.conf"},
  { "modprobe.conf", su_editor_cmd .. " /etc/modprobe.d/modprobe.conf"},
  { "pacman.conf", su_editor_cmd .. " /etc/pacman.conf"},
  { ".vimrc", editor_cmd .. " /home/roughl/.vimrc"},
}

mybluetoothmenu = {
  { "enable", sudo_bash .. '-c "echo enable > /proc/acpi/ibm/bluetooth"'},
  { "disable", sudo_bash .. '-c "echo disable > /proc/acpi/ibm/bluetooth"'},
}
...
awful.rules.rules = {
    ...
    { rule = { instance = "firefox" }, properties = { floating = true, tag = tags[1][1] } },
    -- Set vimprobable to tag 1 of screen 1
    { rule = { class = "vimprobable" }, properties = { tag = tags[1][1] } },
